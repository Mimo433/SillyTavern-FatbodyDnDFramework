...
function scaleImageTo512Square(dataUrl) {
    return new Promise((resolve, reject) => {
        const img = new Image();
        img.onload = () => {
 ...
 */
async function generatePortraitPrompt(entityName) {
    const s = getSettings();
    const ctx = SillyTavern.getContext();
 ...
 */
async function showPortraitPromptPopup(prompt, entityName, localApply, refresh) {
    const ctx = SillyTavern.getContext();
    const s = getSettings();
 ...
 */
async function ensurePollinationsKey() {
    const s = getSettings();
 ...
 */
async function generateWithPollinations(prompt, entityName, localApply, refresh) {
